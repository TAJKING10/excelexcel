...
const TAX_BRACKETS_2025 = {
  // Class 1 - Single (from barème 2025 Excel - Monthly Withholding)
  "1": [
    [0, 1185, 0.0000, 0.0000],
    [1190, 1370, 0.0800, 95.0000],
    [1375, 1555, 0.0900, 108.7125],
    [1560, 1735, 0.1000, 124.2625],
    [1740, 1920, 0.1100, 141.6500],
    [1925, 2105, 0.1200, 160.8750],
    [2110, 2295, 0.1400, 203.0000],
    [2300, 2485, 0.1600, 248.9500],
    [2490, 2680, 0.1800, 298.7250],
    [2685, 2870, 0.2000, 352.3250],
    [2875, 3060, 0.2200, 409.7500],
    [3065, 3250, 0.2400, 471.0000],
    [3255, 3445, 0.2600, 536.0750],
    [3450, 3635, 0.2800, 604.9750],
    [3640, 3825, 0.3000, 677.7000],
    [3830, 4015, 0.3200, 754.2500],
    [4020, 4210, 0.3400, 834.6250],
    [4215, 4400, 0.3600, 918.8250],
    [4405, 4590, 0.3800, 1006.8500],
  ...
  "1A": [
    ...
    [2295, 2435, 0.1000, 229.0000],
    [2440, 2580, 0.1125, 259.4625],
    [2585, 2730, 0.1250, 291.7625],
    [2735, 2875, 0.1375, 325.9000],
    [2880, 3025, 0.1500, 361.8750],
    [3030, 3175, 0.1750, 437.5000],
    [3180, 3330, 0.2000, 516.9500],
    [3335, 3480, 0.2250, 600.2250],
    [3485, 3635, 0.2500, 687.3250],
    [3640, 3790, 0.2750, 778.2500],
    [3795, 3940, 0.3000, 873.0000],
    [3945, 4095, 0.3250, 971.5750],
    [4100, 4245, 0.3500, 1073.9750],
    [4250, 4400, 0.3750, 1180.2000],
  ...
  "2": [
    ...
    [7955, 8335, 0.3400, 1640.3500],
    [8340, 8715, 0.3600, 1807.0500],
    [8720, 9100, 0.3800, 1981.4000],
...
export function calculateIncomeTax(
  ...
  // Apply surcharge based on income threshold (from barème 2025)
  // Class 1: ≤ 12,585 € → 7% surcharge; > 12,585 € → (I × 0.09 - 73.78)
  // Class 1A: ≤ 12,585 € → 7% surcharge; > 12,585 € → (I × 0.09 - 73.78)
  // Class 2: ≤ 25,085 € → 7% surcharge; > 25,085 € → (I × 0.09 - 155.3)

  const surchargeThreshold = classKey === '2' ? 25085 : 12585;
  const surchargeDeduction = classKey === '2' ? 155.3 : 73.78;

  if (revenuImposable <= surchargeThreshold) {
    // Low income: apply 7% surcharge
    impot = impot * 1.07;
  } else {
    // High income: apply (I × 0.09 - deduction) surcharge
    const surcharge = impot * 0.09 - surchargeDeduction;
    impot = impot + surcharge;
  }

  // Round to lower multiple of 0.10 € (floor rounding per barème instructions)
  // "arrondir au multiple inférieur de 10 cents"
  impot = Math.floor(impot * 10) / 10;
